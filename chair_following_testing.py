...
# SET THIS TO THE CHAIR HEIGHT BEFORE RUNNING
GOAL_HEIGHT = 0.85           # [m]
...
def handle_test():
    global COMMAND
    COMMAND = 'test'
    print('Test Requested. Following chair.')
...
class CommNode(Node):
    ...
    def run_yolo(self):
        ...
        results = self.model(frame_undistorted, imgsz=160, conf=0.3, verbose=False)
        ...
        for box in results[0].boxes:
            if int(box.cls[0]) == 56:
                ...
                if area > max_area:
                    max_area = area
                    largest_chair = (x1, y1, x2, y2, float(box.conf[0]))

        if largest_chair is not None:
            self.latest_detection = largest_chair[:4]
            self.last_detection_time = self.get_clock().now()

            x1, y1, x2, y2, conf = largest_chair
            center_x = (x1 + x2) / 2.0
            center_y = (y1 + y2) / 2.0
            box_height = y2 - y1

            cv2.rectangle(annotated, (x1, y1), (x2, y2), (0, 255, 0), 3)
            cv2.putText(annotated, f"LARGEST conf={conf:.2f}",
            ...
            cv2.putText(annotated, f"box_h={box_height}px",
        ...
        chair_count = sum(1 for box in results[0].boxes if int(box.cls[0]) == 56)
        cv2.putText(annotated, f"Chairs: {chair_count}",
...
def run_test_mode(node):
    ...
    while rclpy.ok():
        ...
        if detection_lost:
            print("NO CHAIR DETECTED; would hold position if flying")
        ...
        print(
            f"CHAIR DETECTED"
            f"box height={box_height_px:.0f}px | "
...
def main(args=None):
    ...
    node.get_logger().info(
        ...
        f"goal z={goal_pos.pose.position.z:.3f} (chair height={GOAL_HEIGHT}m)"
    )
    ...
    while rclpy.ok():
        ...
        if MODE == WAIT:
            ...
            vel_msg.header.stamp = node.get_clock().now().to_msg()
            ...
            if detection_lost:
                ...
                node.get_logger().warning(
                    "Chair not detected. Holding position.",
                    throttle_duration_sec=2.0,
                    clock=node.get_clock()
                )
                cmd.pose.position.x = node.odom_pose.pose.position.x
                cmd.pose.position.y = node.odom_pose.pose.position.y
                cmd.pose.position.z = node.ground_z + GOAL_HEIGHT
                cmd.header.stamp = node.get_clock().now().to_msg()
            ...
            distance_error = node.filtered_distance - FOLLOW_DISTANCE
            vx = float(np.clip(distance_error * K_FORWARD, -MAX_VX, MAX_VX))
            yaw_error = IMAGE_WIDTH / 2.0 - center_x
            v_yaw = float(np.clip(yaw_error * K_YAW, -MAX_VYAW, MAX_VYAW))
            ...
            node.get_logger().info(
                f"Following: vx={vx:.2f} v_yaw={v_yaw:.2f} | "
                f"raw={raw_distance:.2f}m filtered={node.filtered_distance:.2f}m "
                f"error={distance_error:.2f}m | "
                f"box_h={box_height_px:.0f}px center_x={center_x:.0f}",
                throttle_duration_sec=1.0,
                clock=node.get_clock()
            )
