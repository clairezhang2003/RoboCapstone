    ...
    while rclpy.ok():
        ...
        if MODE == WAIT:
            ...
            if detection_lost:
                ...
                # Tag is valid and detected
                # Calculate horizontal offset from image center to move toward tag
                # center_x is in pixels, IMAGE_WIDTH is 640
                # Normalize to [-1, 1] where 0 is centered
                pixel_error = center_x - (IMAGE_WIDTH / 2.0)  # negative = tag left, positive = tag right
                norm_error = pixel_error / (IMAGE_WIDTH / 2.0)  # now in [-1, 1]
                
                # Convert pixel error to lateral movement
                # This is a simple proportional controller
                K_LATERAL = 0.5  # [m] per normalized error
                lateral_offset = norm_error * K_LATERAL
                
                # Forward/back: move toward desired distance
                distance_error = node.filtered_distance - FOLLOW_DISTANCE
                K_DISTANCE = 0.3  # proportional gain
                ...
                MAX_LATERAL = 2.0  # [m] don't chase more than 2m sideways
                target_x = np.clip(target_x, node.odom_pose.pose.position.x - MAX_LATERAL, 
                                node.odom_pose.pose.position.x + MAX_LATERAL)
                target_y = np.clip(target_y, node.odom_pose.pose.position.y - MAX_LATERAL, 
